...
{
    [TestClass]
    public class RepositoryEagerLoadingTests : BaseRepositoryTest
    {
        ...
        public async Task CreatePlaythrough_MustHaveCorrectProperties()
        {
            ...
            //
            //
            //await _playthroughRepository.GetSingleAsync(x => x.Id == playThrough.Id);
        ...
        public async Task GetPlaythrough_MustHaveAllLevelsOfParagraphs()
        {
            ...
            var dbPlayThrough = await _playthroughRepository.GetSingleAsync(
                filter: plthru => plthru.Id == playThrough.Id
                , include: x => x.Include(x => x.StartParagraph).ThenInclude(x => x.PlaythroughStats)
                                .Include(x => x.StartParagraph).ThenInclude(x => x.PlaythroughStats).ThenInclude(x => x.Stat)
                                .Include(x => x.StartParagraph).ThenInclude(x => x.ToParagraph)
                                .Include(x => x.StartParagraph).ThenInclude(x => x.ToParagraph).ThenInclude(x => x.PlaythroughStats)
                                .Include(x => x.StartParagraph).ThenInclude(x => x.ToParagraph).ThenInclude(x => x.PlaythroughStats).ThenInclude(x => x.Stat)
                                .Include(x => x.StartParagraph).ThenInclude(x => x.ToParagraph).ThenInclude(x => x.ToParagraph)
                                .Include(x => x.StartParagraph).ThenInclude(x => x.ToParagraph).ThenInclude(x => x.ToParagraph).ThenInclude(x => x.PlaythroughStats)
                    );
